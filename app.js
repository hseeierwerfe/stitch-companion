...
const DEFAULT_STATE = {
    hero: {
        ...
        equipment: {
            melee: null,
            ranged: null,
            armor: { id: 1, name: "Sträflingsklamotten", value: 0, chapter: 0, effect: "Aura +3 Rüstung", category: "Rüstung", icon: "shield", image: "bilder_karten/ruestungen/kapitel_1_und_2/straeflingsklamotten.png" },
        ...
        inventory: [
            { id: 1, name: "Sträflingsklamotten", value: 0, chapter: 0, effect: "Aura +3 Rüstung", category: "Rüstung", icon: "shield", image: "bilder_karten/ruestungen/kapitel_1_und_2/straeflingsklamotten.png" }
        ],
        quests: [],
        chronik: [],
        deactivatedQuests: [],
...
HEROES.forEach(h => {
    if (h.image) {
        let path = h.image.replace("../../", "").replace("Bilder Karten", "bilder_karten");
...
Object.values(CAMP_DATA).forEach(camp => {
    camp.npcs.forEach(npc => {
        if (npc.img) {
            let path = npc.img.replace("Bilder Karten", "bilder_karten");
...
function saveGame() {
    ...
    if (window.SyncManager && typeof window.SyncManager.broadcastHeroState === 'function') {
        window.SyncManager.broadcastHeroState();
    }
}
...
const enemyData = {
    ...
    'beisser':          { name: 'Beißer',          sta: 5, ges: 1, bwg: 6, hp: 20, rust: 1, abilities: 'Panzerbrecher 5 (Roll D6, on 5+ ignore armor)', chapter: 2, class: 2, type: 'allesfresser' },
    // Kapitel 2 – Klasse 3
...
window.confirmEnemySelection = function() {
    ...
    if (!key) return;
    // Admin mode: skip multiMode selection, start 1v1 directly
    if (state.combatPopup.isAdmin) {
        state.combatPopup.multiMode = '1v1';
        confirmMultiCombat();
        return;
    }
...
window.executeEnemyTurn = async function() {
    ...
    const isOccupiedFn = (r, c, ignoreEnemy) => {
        if (r < 0 || r > 14 || c < 0 || c > 14) return true;
        if (sim.hero.pos.r === r && sim.hero.pos.c === c) return true;
        if (sim.hero2 && sim.hero2.pos.r === r && sim.hero2.pos.c === c) return true;
        if (!ignoreEnemy && sim.enemy.pos.r === r && sim.enemy.pos.c === c) return true;
        if (!ignoreEnemy && sim.enemy2 && sim.enemy2.pos.r === r && sim.enemy2.pos.c === c) return true;
        return false;
    };

    for (let i = 0; i < enemyMov; i++) {
        const distR = targetHero.pos.r - activeEnemy.pos.r;
        const distC = targetHero.pos.c - activeEnemy.pos.c;
        const manhattan = Math.abs(distR) + Math.abs(distC);

        // Already adjacent — stop moving, ready to attack
        if (manhattan === 1) break;

        // Already on same cell somehow — stop
        if (manhattan === 0) break;

        const dr = Math.sign(distR);
        const dc = Math.sign(distC);

        // Build priority list of candidate moves: prefer axis that reduces
        // the larger gap first, then the other axis, then the reverse axis
        const candidates = [];
        if (Math.abs(distR) >= Math.abs(distC)) {
            // Prefer vertical first
            if (dr !== 0) candidates.push({ r: activeEnemy.pos.r + dr, c: activeEnemy.pos.c });
            if (dc !== 0) candidates.push({ r: activeEnemy.pos.r, c: activeEnemy.pos.c + dc });
        } else {
            // Prefer horizontal first
            if (dc !== 0) candidates.push({ r: activeEnemy.pos.r, c: activeEnemy.pos.c + dc });
            if (dr !== 0) candidates.push({ r: activeEnemy.pos.r + dr, c: activeEnemy.pos.c });
        }
        // Also try the opposite perpendicular as last resort
        if (dr !== 0) candidates.push({ r: activeEnemy.pos.r - dr, c: activeEnemy.pos.c });
        if (dc !== 0) candidates.push({ r: activeEnemy.pos.r, c: activeEnemy.pos.c - dc });

        // Find first non-occupied candidate
        const selfIsEnemy = (activeEnemy === sim.enemy);
        let moved = false;
        for (const cand of candidates) {
            const occCheck = (r, c) => {
                ...
                if (sim.hero.pos.r === r && sim.hero.pos.c === c) return true;
                if (sim.hero2 && sim.hero2.pos.r === r && sim.hero2.pos.c === c) return true;
                // Don't collide with the other enemy
                if (selfIsEnemy && sim.enemy2 && sim.enemy2.pos.r === r && sim.enemy2.pos.c === c) return true;
                if (!selfIsEnemy && sim.enemy.pos.r === r && sim.enemy.pos.c === c) return true;
                return false;
            };
            if (!occCheck(cand.r, cand.c)) {
                activeEnemy.pos.r = cand.r;
                activeEnemy.pos.c = cand.c;
                hasMoved = true;
                enemyRemainingMov--;
                moved = true;
                render();
        ...
        // Completely blocked — stop trying
        if (!moved) break;
    }
                ...
                <div class="relative z-10 w-full max-w-md px-10 flex flex-col items-center justify-center min-h-screen">
                    ...
                    <nav class="flex flex-col items-center space-y-8 w-full">
                        ...
                        <button onclick="window.openTutorial()" 
                                class="font-headline text-2xl md:text-3xl text-secondary/70 hover:text-secondary active:scale-95 transition-all duration-300 tracking-wide uppercase drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)]">
                            ❓ Tutorial
                        </button>
                    </nav>
...
function openLosingCategory(category) {
    ...
    // Collect names of all currently equipped items to exclude them
    const eq = state.hero.equipment || {};
    const equippedNames = new Set();
    if (eq.melee)  equippedNames.add(eq.melee.name);
    if (eq.ranged) equippedNames.add(eq.ranged.name);
    if (eq.armor)  equippedNames.add(eq.armor.name);
    if (eq.artifacts) {
        if (eq.artifacts.amulet) equippedNames.add(eq.artifacts.amulet.name);
        if (eq.artifacts.ring1)  equippedNames.add(eq.artifacts.ring1.name);
        if (eq.artifacts.ring2)  equippedNames.add(eq.artifacts.ring2.name);
        if (eq.artifacts.others) eq.artifacts.others.forEach(a => equippedNames.add(a.name));
    }
    if (eq.spells) eq.spells.forEach(s => equippedNames.add(s.name));
    
    const items = state.hero.inventory.filter(i => {
        const itemCat = (i.category || "").toLowerCase().trim();
        if (itemCat !== targetCat) return false;
        // Exclude items that are currently equipped
        if (equippedNames.has(i.name)) return false;
        return true;
    });
...
// ─── Sync Modal Global Wrappers ───────────────────────────────────────────────
// Called by onclick handlers in index.html
window.openSyncModal = function() {
    if (window.SyncManager && typeof window.SyncManager.openModal === 'function') {
        window.SyncManager.openModal();
    }
};

window.closeSyncModal = function() {
    if (window.SyncManager && typeof window.SyncManager.closeModal === 'function') {
        window.SyncManager.closeModal();
    }
};
// ─────────────────────────────────────────────────────────────────────────────
